...
const Overview: React.FC<OverviewProps> = ({ cameras, activeCamera, events, summary, onSelectCamera, onAddCameraClick }) => {
              ...
              <div className="divide-y divide-border transition-colors">
                {events.slice(0, 8).map(event => (
                  <div key={event.id} className="p-6 md:p-8 flex items-start space-x-4 md:space-x-6 hover:bg-white/5 transition-all group border-l-4 border-l-transparent hover:border-l-blue-600">
                    <div className={`p-4 md:p-5 rounded-xl md:rounded-[1.5rem] shadow-xl shrink-0 ${
                      event.type === EventType.PERSON ? 'bg-emerald-500/10 text-emerald-500' :
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                        ...
                        <span className={`text-[9px] md:text-[10px] uppercase font-black tracking-widest ${
                          event.confidence === 'high' ? 'text-emerald-500' : 'text-amber-500'
